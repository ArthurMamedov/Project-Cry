...
public:
	...
	void set_crypting_algorithm(const std::shared_ptr<cryptor>& cryptor);
	void encrypt_file(const std::string& path_to_file);
	void decrypt_file(const std::string& path_to_file);
	~file_cryptor() = default;
};
#pragma endregion


#pragma region cryptors
class AES final : public cryptor {
private:
...
#pragma region encryption
	...
	inline void rot_byte(const uint8_t* byte, uint8_t* to);
	inline void sub_byte(uint8_t& byte);
	inline void sub_bytes(uint8_t* state);
	inline void key_extension(const char* key, uint8_t* ext_key);
	inline void add_round_key(uint8_t* state, uint8_t* round_key);
	inline void shift(size_t from, uint8_t* state);
	inline void shift_rows(uint8_t* state);
	inline void mix_colums(uint8_t* state);
...
#pragma region decryption
	...
	inline void inv_sub_byte(uint8_t& byte);
...
class GOST28147_89 final : public cryptor {
private:
	uint32_t round_keys[8];
	inline void round(uint32_t* block32b_1, uint32_t* block32b_2, uint8_t i);
	inline uint32_t substitution_table(uint32_t block32b, uint8_t sbox_row);
	inline void substitution_table_by_4bits(uint8_t* blocks4b, uint8_t sbox_row);
	inline uint32_t join_4bits_to_32bits(uint8_t* blocks4b);
	inline void split_32bits_to_8bits(uint32_t block32b, uint8_t* blocks8b);
	inline void split_256bits_to_32bits(const char* key256b);
...
class BLOWFISH final : public cryptor {
private:
    uint32_t P_block[18] = { 0x2, 0x4, 0x3, 0xF, 0x6, 0xA, 0x8, 0x8, 0x8, 0x5, 0xA, 0x3,  0x0, 0x8, 0xD, 0x3, 0x1, 0x3 };
	uint32_t Sbox[4][256] = {
	...
	inline void split_block(const uint8_t block[8], uint32_t& f, uint32_t& s);
	inline void xor_P_block_with_key(const char* key, const size_t key_size);
	inline void key_extension(const char* key);
...
public:
	static std::shared_ptr<cryptor> create(const char* algorithm, const char* key) {  //TODO: throw the implementation to a separate file.
        ...
        else if (!strcmp(algorithm, "BLOWFISH") || !strcmp(algorithm, "blowfish")) {
